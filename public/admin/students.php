...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? null)) {
        ...
        if ($id === false || $id < 1) {
            ...
            $errors['form'] = 'Impossible de supprimer cet etudiant.';
        }
    }
}
...
<head>
    ...
    <title>Étudiants - UniSoutenance</title>
        ...
        <main class="flex-1 p-8">
            <div class="mb-8 flex items-center justify-between">
                <div>
                    ...
                    <h2 class="mt-2 text-3xl font-bold text-slate-900">Gestion des étudiants</h2>
                </div>
                <a href="students-form.php" class="rounded-lg bg-slate-900 px-4 py-2 font-semibold text-white hover:bg-slate-700">Nouvel étudiant</a>
            </div>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                    <thead class="bg-slate-50 text-slate-600">
                        <tr>
                            ...
                            <th class="px-5 py-3 font-semibold">Filière</th>
                            <th class="px-5 py-3 font-semibold text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200">
                        <?php if ($students === []): ?>
                            <tr><td colspan="5" class="px-5 py-8 text-center text-slate-500">Aucun étudiant enregistré.</td></tr>
                        <?php else: ?>
                            <?php foreach ($students as $student): ?>
                                <tr>
                                    <td class="px-5 py-4 font-medium text-slate-800"><?= e((string) ($student['nom'] ?? '')) ?></td>
                                    <td class="px-5 py-4"><?= e((string) ($student['prenom'] ?? '')) ?></td>
                                    <td class="px-5 py-4"><?= e((string) ($student['matricule'] ?? '')) ?></td>
                                    <td class="px-5 py-4"><?= e((string) ($student['filiere_nom'] ?? '')) ?></td>
                                    <td class="px-5 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <a class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:border-slate-400" href="students-form.php?id=<?= (int) ($student['id'] ?? 0) ?>">Modifier</a>
                                            <form method="post" action="students.php" onsubmit="return confirm('Supprimer cet étudiant ?');">
