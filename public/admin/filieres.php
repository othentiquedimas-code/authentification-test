...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? null)) {
        ...
        if ($id === false || $id < 1) {
            $errors['form'] = 'Identifiant de filiere invalide.';
        } else {
            ...
            $errors['form'] = 'Impossible de supprimer cette filiere.';
        }
    }
}
...
<head>
    ...
    <title>Filieres - UniSoutenance</title>
        ...
        <main class="flex-1 p-8">
            <div class="mb-8 flex items-center justify-between">
                <div>
                    ...
                    <h2 class="mt-2 text-3xl font-bold text-slate-900">Gestion des filières</h2>
                </div>
                <a href="filieres-form.php" class="rounded-lg bg-slate-900 px-4 py-2 font-semibold text-white hover:bg-slate-700">Nouvelle filière</a>
            </div>
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-200">
                        <?php if ($filieres === []): ?>
                            <tr>
                                <td colspan="4" class="px-5 py-8 text-center text-slate-500">Aucune filière enregistrée.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($filieres as $filiere): ?>
                                <tr>
                                    <td class="px-5 py-4 font-medium text-slate-800"><?= e((string) ($filiere['nom'] ?? '')) ?></td>
                                    <td class="px-5 py-4"><?= e((string) ($filiere['code'] ?? '')) ?></td>
                                    <td class="px-5 py-4 text-slate-600"><?= e((string) ($filiere['description'] ?? '')) ?></td>
                                    <td class="px-5 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <a class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:border-slate-400" href="filieres-form.php?id=<?= (int) ($filiere['id'] ?? 0) ?>">Modifier</a>
                                            <form method="post" action="filieres.php" onsubmit="return confirm('Supprimer cette filière ?');">
                                                <input type="hidden" name="csrf_token" value="<?= e(csrfToken()) ?>">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="id" value="<?= (int) ($filiere['id'] ?? 0) ?>">
